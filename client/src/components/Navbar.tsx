...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-sm border-b border-slate-200 py-3" : "bg-transparent py-5"
        }`}
    >
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col space-y-3">
            ...
            {user ? (
              <>
                <Link
                  href={`/${user.role.toLowerCase()}`}
                  className="text-base font-medium text-slate-600 block w-full text-center py-2 border border-slate-200 rounded-lg"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Dashboard
                </Link>
                ...
                <button
                  ...
                  className="text-base font-medium bg-red-600 text-white w-full py-2 rounded-lg"
                >
                  Logout
                </button>
              </>
